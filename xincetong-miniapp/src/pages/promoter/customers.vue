<template>
  <view class="page page-bg">
    ...
    <view class="content">
      ...
      <view class="subtitle">按推广链接聚合的客户</view>
...
const title = ref('客户列表')
const stage = ref('6')
const hint  = ref('按推广链接聚合的客户')

const { shareCode, promoterCode } = useShareCode()

onLoad(() => {
  pageView('promoter/customers')
})
...
function goNext() {
  uni.navigateTo({ url: '/pages/promoter/customer-detail' })
}
</script>
