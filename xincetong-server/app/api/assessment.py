...
from app.services.shared_config import (
    ...
    build_projection_table as _shared_projection_table,
    build_product_matches as _shared_product_matches,
    build_data_hash as _shared_data_hash,
)
from app.services.bank_scorecard import (
    get_bank_focus_slogan as _bank_focus_slogan,
    get_bank_key_tags as _bank_key_tags,
    get_bank_weight as _bank_weight,
)
...
async def submit_assessment(
    ...
    # 1. 跑 6 套独立模型（v7 增量：按 bank_code 选差异化评分）
    product_results = await calculate_scores_by_product(input_data, type_, db, bank_code)
    ...
    return ok({
        ...
        "bank_name": bank_name,  # v7 增量：free 视图带 bank_name（前端 fp-bank-focus 标题用）
        "bank_code": bank_code,  # v7 增量：free 视图带 bank_code
        "overall": overall_dict,
        "one_sentence": one_sentence,
        ...
        # v9 增量：6 产品独立证据链索引（按 product_code 取值）
        "product_breakdown": _build_product_breakdown([r.to_dict() for r in product_results]),
        "is_paid": False,
        "paid_at": None,
        "veto_count": sum(1 for r in product_results if r.veto is not None),
        "dimensions": None,    # v5 P0 已临时回滚（model 字段 + 计算逻辑都已注释）
        # ============ v6 一致性 SSOT（双端 web/H5 共用）============
        "projection_table": _shared_projection_table(
            overall_score, overall_level, limit_min, limit_max, rate_min, rate_max, projection
        ),
        "product_matches": _shared_product_matches(overall_score, products_free_view),
        "ui_config": _shared_ui_config(),
        "data_hash": _shared_data_hash(overall_dict, projection, products_free_view, top_issues),
        # ============ v7 银行差异化评估 ============
        "bank_focus": _bank_focus_slogan(bank_code),  # 短句：用于 UI 顶部展示
        "bank_key_tags": _bank_key_tags(bank_code),    # 标签列表：3~4 个银行特点
    })
...
# 严重度对应的视觉颜色（前端可读 hex）
# v8 极简：严重度颜色从「红/橙/金」改成「深棕/古铜/金」单色阶
_ISSUE_SEV_COLOR = {"high": "#8E6F2C", "mid": "#C9A96E", "low": "#8B7E5E"}
...
async def get_free_result(
    ...
    # v7 增量：从 a.bank_code 算出 bank_name（前端 free.vue 用）
    bank_name = None
    if a.bank_code:
        bank_row = await db.execute(
            select(Bank).where(Bank.code == a.bank_code, Bank.enabled == 1)
        )
        bank_obj = bank_row.scalar_one_or_none()
        if bank_obj:
            bank_name = bank_obj.name
    ...
    return ok({
        "assessment_id": a.id,
        "report_no": a.report_no,
        # v7 增量：bank_name + bank_focus + bank_key_tags + bank_code（前端 fp-bank-focus 用）
        "bank_name": bank_name,
        "bank_code": a.bank_code,
        "bank_focus": _bank_focus_slogan(a.bank_code) if a.bank_code else None,
        "bank_key_tags": _bank_key_tags(a.bank_code) if a.bank_code else [],
        "overall": {
            "score": a.score,
            "level": a.level,
        ...
        # v9 增量：6 产品独立证据链索引（按 product_code 取值）
        "product_breakdown": _build_product_breakdown(a.product_results),
        ...
        # ============ v6 一致性 SSOT（双端 web/H5 共用）============
        "projection_table": _shared_projection_table(
            a.score or 0, a.level or "E",
            a.limit_min or 0, a.limit_max or 0,
            float(a.rate_min) if a.rate_min else 0,
            float(a.rate_max) if a.rate_max else 0,
            a.improvement_projection or None,
        ),
        "product_matches": _shared_product_matches(a.score or 0, a.product_results or []),
        "ui_config": _shared_ui_config(),
        "data_hash": _shared_data_hash(
            {"score": a.score, "level": a.level, "limit_min": a.limit_min, "limit_max": a.limit_max,
             "rate_min": float(a.rate_min) if a.rate_min else 0, "rate_max": float(a.rate_max) if a.rate_max else 0,
             "pass_probability": a.pass_probability},
            a.improvement_projection, a.product_results, a.top_issues,
        ),
    })
...
@router.get("/compare-snapshot/{assessment_id}", summary="v6 一致性快照（双端渲染前校验用）")
async def get_compare_snapshot(
    assessment_id: int = Path(..., ge=1),
    db: AsyncSession = Depends(get_db),
) -> dict:
    """
    返回评估关键字段 + data_hash，双端（web/h5）渲染前调此接口，
    若与本地计算 hash 不一致，提示用户刷新，避免错位。

    返回：
      { assessment_id, overall, projection_table, product_matches,
        ui_config, data_hash, is_paid, updated_at }
    """
    a = await db.get(Assessment, assessment_id)
    if not a:
        raise NotFoundException(f"测评记录 {assessment_id} 不存在")
    ...
    proj_table = _shared_projection_table(
        a.score or 0, a.level or "E",
        a.limit_min or 0, a.limit_max or 0,
        float(a.rate_min) if a.rate_min else 0,
        float(a.rate_max) if a.rate_max else 0,
    ...
    prod_matches = _shared_product_matches(a.score or 0, a.product_results or [])
    ui = _shared_ui_config()
    dh = _shared_data_hash(overall_dict, a.improvement_projection, a.product_results, a.top_issues)
    return ok({
        "assessment_id": a.id,
        "overall": overall_dict,
        "projection_table": proj_table,
        "product_matches": prod_matches,
        "ui_config": ui,
        "data_hash": dh,
        "is_paid": bool(a.is_paid),
        "updated_at": a.updated_at.isoformat() if hasattr(a, "updated_at") and a.updated_at else a.created_at.isoformat() if a.created_at else None,
    })
...
# v6 一致性 SSOT 包装函数（供三处接口调用，避免传参重复）
def _build_product_breakdown(product_results: list | None) -> dict:
    """v9 增量：把 6 个产品结果按 product_code 索引为 dict

    方便前端 O(1) 取值（如 result.product_breakdown.quality_unit.hit_rules），
    避免遍历 list 找对应产品。

    每个 entry 包含 6 产品独立证据链的 5 个核心字段：
      - hit_rules: 命中高分规则 top 3
      - low_rules: 命中低分规则 top 3
      - improve_vars: 提分变量 top 3
      - not_recommend_reason: 不推荐原因（1-2 句）
      - realistic_limit_min/max: 实际可贷金额（按本产品等级折扣后）
    """
    out: dict = {}
    for r in product_results or []:
        code = r.get("product_code") if isinstance(r, dict) else getattr(r, "product_code", None)
        if not code:
            continue
        out[code] = {
            "level": r.get("level", "E") if isinstance(r, dict) else getattr(r, "level", "E"),
            "score": r.get("score", 0) if isinstance(r, dict) else getattr(r, "score", 0),
            "hit_rules": r.get("hit_rules", []) if isinstance(r, dict) else getattr(r, "hit_rules", []),
            "low_rules": r.get("low_rules", []) if isinstance(r, dict) else getattr(r, "low_rules", []),
            "improve_vars": r.get("improve_vars", []) if isinstance(r, dict) else getattr(r, "improve_vars", []),
            "not_recommend_reason": (
                r.get("not_recommend_reason", "") if isinstance(r, dict)
                else getattr(r, "not_recommend_reason", "")
            ),
            "realistic_limit_min": (
                r.get("realistic_limit_min", 0) if isinstance(r, dict)
                else getattr(r, "realistic_limit_min", 0)
            ),
            "realistic_limit_max": (
                r.get("realistic_limit_max", 0) if isinstance(r, dict)
                else getattr(r, "realistic_limit_max", 0)
            ),
        }
    return out


def _shared_projection_table(score, level, limit_min, limit_max, rate_min, rate_max, projection):
    try:
        return build_projection_table(score, level, limit_min, limit_max, rate_min, rate_max, projection)
    except Exception as e:
        logger.warning(f"build_projection_table 失败: {e}")
        return []


def _shared_product_matches(score, product_results):
    try:
        return build_product_matches(score, product_results)
    except Exception as e:
        logger.warning(f"build_product_matches 失败: {e}")
        return []


def _shared_ui_config():
    try:
        return build_ui_config()
    except Exception as e:
        logger.warning(f"build_ui_config 失败: {e}")
        return {}


def _shared_data_hash(*fields):
    try:
        return build_data_hash(*fields)
    except Exception:
        return ""
