<template>
  <view class="page page-bg">
    ...
    <view class="content">
      <view class="title">推广员工作台</view>
      <view class="subtitle">今日/累计数据</view>
...
const title = ref('推广员工作台')
const stage = ref('6')
const hint  = ref('今日/累计数据')

const { shareCode, promoterCode } = useShareCode()

onLoad(() => {
  pageView('promoter/dashboard')
})
...
function goNext() {
  uni.navigateTo({ url: '/pages/promoter/customers' })
}
</script>
